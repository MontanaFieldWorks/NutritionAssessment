...
export default function App() {
  const [clients, setClients] = useState<Client[]>(() => {
    const saved = localStorage.getItem("sharpe_clients");
    if (saved) {
      try {
  ...
  const [activeClientId, setActiveClientId] = useState<string>(() => {
    return localStorage.getItem("sharpe_active_client_id") || "reuvein-mittler";
  });
  const [journals, setJournals] = useState<Record<string, JournalDay[]>>(() => {
    const saved = localStorage.getItem("sharpe_journals");
    if (saved) {
      try {
        return JSON.parse(saved);
      } catch (e) {
    ...
    return SEEDED_JOURNAL;
  });
  const [reports, setReports] = useState<Record<string, ClientReport[]>>(() => {
    const saved = localStorage.getItem("sharpe_reports");
    if (saved) {
      try {
        return JSON.parse(saved);
      } catch (e) {
  ...
  const [selectedReportId, setSelectedReportId] = useState<string | null>(() => {
    const defaultReports = JSON.parse(localStorage.getItem("sharpe_reports") || "null") || SEEDED_REPORTS;
    const activeId = localStorage.getItem("sharpe_active_client_id") || "reuvein-mittler";
    const clientReps = defaultReports[activeId] || [];
    return clientReps.length > 0 ? clientReps[0].id : null;
  });
  const [activeAnalysis, setActiveAnalysis] = useState<NutritionAnalysis | null>(() => {
    const defaultReports = JSON.parse(localStorage.getItem("sharpe_reports") || "null") || SEEDED_REPORTS;
    const activeId = localStorage.getItem("sharpe_active_client_id") || "reuvein-mittler";
    const clientReps = defaultReports[activeId] || [];
    return clientReps.length > 0 ? clientReps[0].analysis : null;
  });

  // Next week's custom target parameters state
  const [nextWeekTargets, setNextWeekTargets] = useState<Record<string, { calories: number; proteinPct: number; carbsPct: number; fatPct: number; notes?: string }>>(() => {
    const saved = localStorage.getItem("sharpe_next_week_targets");
    if (saved) {
      try {
        return JSON.parse(saved);
      } catch (e) {
  ...
  React.useEffect(() => {
    localStorage.setItem("sharpe_clients", JSON.stringify(clients));
  }, [clients]);

  React.useEffect(() => {
    localStorage.setItem("sharpe_active_client_id", activeClientId);
  }, [activeClientId]);

  React.useEffect(() => {
    localStorage.setItem("sharpe_journals", JSON.stringify(journals));
  }, [journals]);

  React.useEffect(() => {
    localStorage.setItem("sharpe_reports", JSON.stringify(reports));
  }, [reports]);

  React.useEffect(() => {
    localStorage.setItem("sharpe_next_week_targets", JSON.stringify(nextWeekTargets));
  }, [nextWeekTargets]);

  // Snapshot/Saved weekly logs state
  const [savedLogs, setSavedLogs] = useState<{
    ...
    nextWeekTarget: { calories: number; proteinPct: number; carbsPct: number; fatPct: number };
  }[]>(() => {
    const saved = localStorage.getItem("sharpe_saved_logs");
    if (saved) {
      try {
        return JSON.parse(saved);
      } catch (e) {
  ...
  const [vaultNotification, setVaultNotification] = useState<{ message: string; type: "success" | "error" } | null>(null);

  const triggerNotification = (message: string, type: "success" | "error" = "success") => {
    setVaultNotification({ message, type });
    setTimeout(() => {
  ...
  React.useEffect(() => {
    localStorage.setItem("sharpe_saved_logs", JSON.stringify(savedLogs));
  }, [savedLogs]);
...
0.4 g`;
  ...
  const commitPastedJournal = () => {
    ...
    // Auto-archive current week to snapshot vault if it has items, so no data is lost!
    if (existingJournal.length > 0) {
      ...
      const clientSanitized = activeClient.name.toLowerCase().replace(/[^a-z0-9]/g, "-");
      const uniqueId = `archive_text_${clientSanitized}_${dateStr}_${Date.now()}`;
      
      const backupTitle = `${activeClient.name} - Archived week of ${mondayText} (Auto-Saved prior to text import)`;
      ...
      const backupSnapshot = {
        ...
        weeksText: backupTitle,
        client: activeClient,
        journal: [...existingJournal],
        analysis: activeAnalysis,
        nextWeekTarget: getActiveNextWeekTarget()
      };

      setSavedLogs(prev => [backupSnapshot, ...prev]);
    }

    // Set the client's journal EXCLUSIVELY to the parsed Everfit days (creating a new log/week)
    const updatedJournal = [...parsedDraftDays];
    ...
    // Reset current analysis state since this is a new log entry
    setActiveAnalysis(null);
    setSelectedReportId(null);
    ...
    const feedbackMsg = existingJournal.length > 0 
      ? "Previous week archived to Log Vault. Created a brand new weekly log from the active Everfit import!"
      : "Created a brand new weekly log from the active Everfit import!";
    
    setPasteFeedback(feedbackMsg);
    triggerNotification(feedbackMsg, "success");
  ...
  const commitCSVImport = () => {
    ...
    if (existingJournal.length > 0) {
      ...
      const dateStr = new Date().toISOString().split('T')[0];
      const clientSanitized = activeClient.name.toLowerCase().replace(/[^a-z0-9]/g, "-");
      const uniqueId = `archive_csv_${clientSanitized}_${dateStr}_${Date.now()}`;
      
      const backupTitle = `${activeClient.name} - Archived week of ${mondayText} (Auto-Saved prior to CSV import)`;
    ...
    // Set the client's journal EXCLUSIVELY to the parsed CSV days (creating a new log/week)
    const updatedJournal = [...newDays];
    ...
    const feedbackMsg = existingJournal.length > 0
      ? `Previous log archived. Successfully started a new week with CSV! Lodged ${newDays.length} days of data.`
      : `Successfully started a new week with CSV! Lodged ${newDays.length} days of data.`;
  ...
  const getActiveNextWeekTarget = () => {
    const defaultTarget = {
      calories: activeClient.goalCalories,
      proteinPct: 35,
      carbsPct: 35,
      fatPct: 30,
      notes: ""
    };
    const target = nextWeekTargets[activeClient.id] || defaultTarget;
    return {
      ...defaultTarget,
      ...target
    };
  ...
  const updateNextWeekTargetField = (field: "calories" | "proteinPct" | "carbsPct" | "fatPct", value: number) => {
    const current = getActiveNextWeekTarget();
    setNextWeekTargets(prev => ({
      ...prev,
      [activeClient.id]: {
        ...current,
        [field]: value
  ...
  const updateNextWeekTargetNotes = (notes: string) => {
    const current = getActiveNextWeekTarget();
    setNextWeekTargets(prev => ({
      ...prev,
      [activeClient.id]: {
  ...
  const exportWeeklyLogJson = () => {
    ...
    const clientSanitized = activeClient.name.toLowerCase().replace(/\s+/g, "_");
    const mondayText = getMondayDateText().replace(/\s+/g, "_");
    ...
    downloadAnchor.setAttribute(
      "download",
      `SharpeFN_Log_${clientSanitized}_${mondayText}.json`
    );
  ...
  const importWeeklyLogJson = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    fileReader.onload = (event) => {
      try {
        ...
        if (!parsed.client || !parsed.journal) {
          alert("Invalid file format. Please upload a valid SharpeFN weekly log JSON file.");
          return;
        }
        
        const importedClient: Client = parsed.client;
        
        // 1. Update/Add Clients
        setClients(prev => {
          const exists = prev.some(c => c.id === importedClient.id);
          if (!exists) {
            return [...prev, importedClient];
          } else {
            return prev.map(c => c.id === importedClient.id ? importedClient : c);
          }
        });
        
        // 2. Select this Client
        setActiveClientId(importedClient.id);
        
        // 3. Update Journals
        setJournals(prev => ({
          ...prev,
          [importedClient.id]: parsed.journal
        }));
        
        // 4. Update Analysis
        if (parsed.analysis) {
          setActiveAnalysis(parsed.analysis);
          
          const reportId = `r-${importedClient.id}-${Date.now()}`;
          const newReport: ClientReport = {
            id: reportId,
            clientId: importedClient.id,
            dateRange: getMondayDateText(),
          ...
          setReports(prev => ({
            ...prev,
            [importedClient.id]: [newReport, ...(prev[importedClient.id] || [])]
          }));
        } else {
          setActiveAnalysis(null);
          setSelectedReportId(null);
        ...
        // 5. Update next week goals
        if (parsed.nextWeekTarget) {
          setNextWeekTargets(prev => ({
            ...prev,
            [importedClient.id]: parsed.nextWeekTarget
          }));
        }
        
        triggerNotification(`Successfully imported log for ${importedClient.name}!`, "success");
      } catch (err) {
        console.error(err);
        triggerNotification("Failed to parse the JSON file. Ensure it is a valid SharpeFN log file.", "error");
      }
    };
  ...
  // Save Snapshot of current state with a unique id
  const saveWeeklySnapshot = (customTitle?: string) => {
    const mondayText = getMondayDateText();
    const formattedDate = new Date().toLocaleString("en-US", {
      month: "short",
      day: "numeric",
    ...
    // Unique ID layout: name_date_timestamp
    const dateStr = new Date().toISOString().split('T')[0];
    const clientSanitized = activeClient.name.toLowerCase().replace(/[^a-z0-9]/g, "-");
    const uniqueId = `${clientSanitized}_${dateStr}_${Date.now()}`;
    
    const defaultTitle = `${activeClient.name} - Week of ${mondayText} (Saved: ${formattedDate})`;
    ...
    const newSnapshot = {
      ...
      clientName: activeClient.name,
      dateCreated: formattedDate,
      weeksText: snapshotTitle, // this is the display label!
      client: activeClient,
      journal: clientJournal,
      analysis: activeAnalysis,
      nextWeekTarget: getActiveNextWeekTarget()
    ...
    setSavedLogs(prev => [newSnapshot, ...prev]);
    triggerNotification(`Saved Snapshot Log: "${snapshotTitle}"`, "success");
    return uniqueId;
  };

  // Recall Snapshot log and set to current active state
  const recallWeeklySnapshot = (snapshotId: string) => {
    const log = savedLogs.find(l => l.id === snapshotId);
    if (!log) {
      triggerNotification("Saved log record not found.", "error");
      return;
    }

    // 1. Restore Client
    setClients(prev => {
      const exists = prev.some(c => c.id === log.client.id);
      if (!exists) {
        return [...prev, log.client];
      } else {
        return prev.map(c => c.id === log.client.id ? log.client : c);
      }
    });

    // 2. Load the client ID
    setActiveClientId(log.client.id);

    // 3. Restore Journal
    setJournals(prev => ({
      ...prev,
      [log.client.id]: log.journal
    }));

    // 4. Restore Analysis
    setActiveAnalysis(log.analysis);

    // 5. Restore next week targets
    if (log.nextWeekTarget) {
      setNextWeekTargets(prev => ({
        ...prev,
        [log.client.id]: log.nextWeekTarget
      }));
    }

    triggerNotification(`Recalled Log Record for ${log.clientName}`, "success");
  };

  // Delete a saved snapshot log from local registry
  const deleteWeeklySnapshot = (snapshotId: string, e?: React.MouseEvent) => {
    if (e) {
      e.stopPropagation();
      e.preventDefault();
    }
    ...
    if (confirm(`Are you sure you want to delete the saved record: "${log.weeksText}"?`)) {
      setSavedLogs(prev => prev.filter(l => l.id !== snapshotId));
      triggerNotification("Saved record deleted from vault.", "success");
    }
  };
  ...
  return (
    <div className="min-h-screen bg-white text-black font-sans selection:bg-blue-100 selection:text-blue-900">
      ...
      {/* ----------------- DIGITAL SNAPSHOT REGISTRY & VAULT (HIDDEN ON PRINT) ----------------- */}
      <div className="print:hidden bg-slate-950 text-white border-b border-slate-800 py-3 px-4 sm:px-6 lg:px-8">
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="text-xs font-black uppercase tracking-widest text-emerald-400">Log Snapshot Vault</span>
                <span className="text-[9px] uppercase font-bold bg-slate-800 text-gray-300 px-1.5 py-0.5 rounded">Local client records backup</span>
              </div>
              <p className="text-[10px] text-gray-400 mt-0.5">Capture weekly tracker sheets, AI feedback summaries and targets, and toggle back any time!</p>
            </div>
          </div>
          ...
          <div className="flex flex-wrap items-center gap-3">
            ...
            {/* Direct Instant Snapshot Save Input & Button */}
            <div className="flex items-center bg-slate-800 rounded-lg p-1 border border-slate-700 shadow-inner">
              <input
                type="text"
                placeholder={`${activeClient.name} - Week of ${getMondayDateText()}`}
                value={customSnapshotTitle}
                onChange={(e) => setCustomSnapshotTitle(e.target.value)}
                className="bg-transparent text-xs text-white placeholder-gray-400 font-bold px-3 py-1.5 focus:outline-none w-52 sm:w-64"
              />
              <button
                onClick={() => {
                  saveWeeklySnapshot(customSnapshotTitle);
                  setCustomSnapshotTitle("");
                }}
                className="flex items-center gap-1 bg-emerald-600 hover:bg-emerald-500 text-white text-xs font-bold px-3.5 py-1.5 rounded-md transition-colors cursor-pointer"
              >
            ...
            <div className="flex items-center gap-1.5 bg-slate-800 rounded-lg p-1 border border-slate-700">
              <select
                onChange={(e) => {
                  if (e.target.value) {
                    recallWeeklySnapshot(e.target.value);
                  }
                }}
                defaultValue=""
                className="bg-transparent text-xs text-gray-200 font-medium p-1.5 focus:outline-none cursor-pointer max-w-[200px] md:max-w-xs"
              >
                <option value="" disabled className="bg-slate-905 text-gray-400 font-semibold text-xs">📂 Choose snapshot...</option>
                {savedLogs.map((log) => (
                  <option key={log.id} value={log.id} className="bg-slate-900 text-white text-xs py-1">
                    {log.weeksText}
                  </option>
                ))}
              </select>
              ...
              {savedLogs.length > 0 && (
                <div className="flex items-center pr-1 text-[10px] text-gray-400 font-bold uppercase">
                  <span>({savedLogs.length})</span>
                </div>
              )}
            </div>

            {/* Delete saved selector direct helper */}
            {savedLogs.length > 0 && (
              <select
                onChange={(e) => {
                  if (e.target.value) {
                    deleteWeeklySnapshot(e.target.value);
                    e.target.value = "";
                  }
                }}
                defaultValue=""
                className="bg-red-950/40 hover:bg-red-900/50 border border-red-700/40 text-red-100 hover:text-white text-[10px] font-bold p-2 rounded-lg focus:outline-none cursor-pointer transition-all"
              >
                <option value="" disabled className="bg-slate-905 text-gray-400 font-bold text-[10px]">🗑️ Delete snapshot...</option>
                {savedLogs.map((log) => (
                  <option key={log.id} value={log.id} className="bg-slate-900 text-red-200 text-xs">
                    ❌ Delete: {log.weeksText.split(" (Saved:")[0]}
                  </option>
                ))}
              </select>
            )}
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 print:hidden">
        ...
        {activeSubTab === "macros-report" && (
          <div className="space-y-8">
            ...
            {/* ----------------- FOR NEXT WEEK: METABOLIC TARGETS & RECORDS VAULT ----------------- */}
            <div className="bg-white text-black rounded-2xl p-6 md:p-8 border border-gray-250 shadow-sm mt-8">
              <div className="pb-4 border-b border-gray-200">
                <h2 className="text-xl md:text-2xl font-black tracking-tight mt-1 text-slate-900 uppercase">
                  FOR NEXT WEEK: Target Metabolic Metrics & Records
                </h2>
                <p className="text-xs text-gray-505 font-bold uppercase tracking-wider mt-1">Prescribe target guidelines and backup client logs locally</p>
              </div>

              <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 mt-6">
                {/* Left side: Parametric metric editors */}
                <div className="lg:col-span-6 space-y-4">
                  <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest border-b pb-1">Edit Target Guidelines</h3>
                  
                  <div>
                    <label className="block text-xs font-bold text-gray-700 uppercase mb-1">Calories Budget (kcal / day)</label>
                    <input
                      type="number"
                      value={getActiveNextWeekTarget().calories}
                      onChange={(e) => updateNextWeekTargetField("calories", parseInt(e.target.value) || 0)}
                      className="w-full bg-gray-50 border border-gray-300 rounded-xl p-3 text-sm font-bold focus:ring-1 focus:ring-slate-950 focus:outline-none"
                    />
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-3">
                    <div>
                      ...
                      <input
                        type="number"
                        value={getActiveNextWeekTarget().proteinPct}
                        onChange={(e) => updateNextWeekTargetField("proteinPct", parseInt(e.target.value) || 0)}
                        className="w-full bg-gray-50 border border-gray-300 rounded-xl p-2.5 text-center text-sm font-bold focus:ring-1 focus:ring-slate-950"
                      />
                    </div>
                    <div>
                      <label className="block text-xs font-semibold text-gray-650 mb-1">Carbs (%)</label>
                      <input
                        type="number"
                        value={getActiveNextWeekTarget().carbsPct}
                        onChange={(e) => updateNextWeekTargetField("carbsPct", parseInt(e.target.value) || 0)}
                        className="w-full bg-gray-50 border border-gray-300 rounded-xl p-2.5 text-center text-sm font-bold focus:ring-1 focus:ring-slate-950"
                      />
                    </div>
                    <div>
                      ...
                      <input
                        type="number"
                        value={getActiveNextWeekTarget().fatPct}
                        onChange={(e) => updateNextWeekTargetField("fatPct", parseInt(e.target.value) || 0)}
                        className="w-full bg-gray-50 border border-gray-300 rounded-xl p-2.5 text-center text-sm font-bold focus:ring-1 focus:ring-slate-950"
                      />
                    </div>
                  </div>

                  {/* Validation Split Sum check */}
                  {(() => {
                    const target = getActiveNextWeekTarget();
                    const sum = target.proteinPct + target.carbsPct + target.fatPct;
                    return (
                      <div className="text-xs flex items-center justify-between font-bold px-3 py-2 rounded-lg bg-gray-50 border border-gray-150">
                        <span>Total Percent sum: <span className={sum === 100 ? "text-emerald-600" : "text-amber-605"}>{sum}%</span></span>
                        <span className="text-[10px] text-gray-400 font-semibold uppercase">
                          {sum === 100 ? "Valid Split (100%)" : "Must sum to 100%!"}
                        </span>
                      </div>
                    );
                  })()}
                  ...
                  <div className="pt-2">
                    <label className="block text-xs font-bold text-gray-700 uppercase mb-1">Coaching Targets Notes</label>
                    <textarea
                      placeholder="Enter specific next-week targets instructions, focus areas or food lists here..."
                      value={getActiveNextWeekTarget().notes || ""}
                      onChange={(e) => updateNextWeekTargetNotes(e.target.value)}
                      rows={3}
                      className="w-full bg-gray-50 border border-gray-300 rounded-xl p-3 text-xs font-semibold text-slate-800 focus:ring-1 focus:ring-slate-950 focus:outline-none placeholder-gray-400 resize-y"
                    />
                  </div>
                </div>

                {/* Right side: Summary display & digital storage actions */}
                <div className="lg:col-span-6 space-y-6 flex flex-col justify-between font-medium">
                  <div>
                    <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest border-b pb-1 mb-4">Prescribed Metrics Summary</h3>
                    {(() => {
                      const target = getActiveNextWeekTarget();
                      const pGrams = Math.round((target.calories * target.proteinPct / 100) / 4);
                      const cGrams = Math.round((target.calories * target.carbsPct / 100) / 4);
                      const fGrams = Math.round((target.calories * target.fatPct / 100) / 9);

                      return (
                        <div className="grid grid-cols-2 gap-4">
                          <div className="p-3 bg-slate-950 text-white rounded-xl col-span-2 flex justify-between items-center shadow-xs">
                            <span className="text-[10px] text-gray-300 font-bold uppercase tracking-wider">Daily Calories target</span>
                            <span className="text-lg font-black">{target.calories.toLocaleString()} kcal/day</span>
                          </div>

                          <div className="p-3 bg-gray-50 border border-gray-200 rounded-xl">
                            <span className="text-[9px] text-blue-600 font-bold block uppercase tracking-wide">Protein Budget</span>
                            <span className="text-base font-black text-slate-900 block mt-1">{target.proteinPct}%</span>
                            <span className="text-xs text-slate-500 font-bold mt-0.5 block">{pGrams} grams / day</span>
                          </div>

                          <div className="p-3 bg-gray-50 border border-gray-200 rounded-xl">
                            <span className="text-[9px] text-emerald-700 font-bold block uppercase tracking-wide">Carbohydrate Budget</span>
                            <span className="text-base font-black text-slate-900 block mt-1">{target.carbsPct}%</span>
                            <span className="text-xs text-slate-500 font-bold mt-0.5 block">{cGrams} grams / day</span>
                          </div>

                          <div className="p-3 bg-gray-50 border border-gray-200 rounded-xl col-span-2">
                            <span className="text-[9px] text-amber-600 font-bold block uppercase tracking-wide">Dietary Fat Budget</span>
                            <span className="text-base font-black text-slate-900 mt-1 flex justify-between items-center">
                              <span>{target.fatPct}%</span>
                              <span className="text-xs text-slate-500 font-bold">{fGrams} grams / day</span>
                            </span>
                          </div>
                        </div>
                      );
                  ...
                  {/* Digital Backups Vault */}
                  <div className="pt-4 border-t border-gray-250">
                    <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-3">Digital Client Record Vault</h3>
                    <div className="grid grid-cols-2 gap-3">
                      <button
                        onClick={exportWeeklyLogJson}
                        className="flex items-center justify-center gap-2 px-4 py-2.5 bg-slate-900 hover:bg-slate-800 text-white text-xs font-extrabold rounded-xl transition-all cursor-pointer shadow-sm uppercase tracking-wider"
                      >
                        <Download className="h-4 w-4" />
                        Save Weekly Log to PC
                      </button>

                      <label
                        className="flex items-center justify-center gap-2 px-4 py-2.5 border border-slate-300 hover:bg-gray-50 text-slate-800 text-xs font-extrabold rounded-xl transition-all cursor-pointer shadow-xs uppercase tracking-wider text-center"
                      >
                        <Upload className="h-4 w-4 text-emerald-600 animate-pulse" />
                        <span>Upload Saved PC Log</span>
                        <input
                          type="file"
                          accept=".json"
                          onChange={importWeeklyLogJson}
                          className="hidden"
                        />
                      </label>
                    </div>
      ...
      {vaultNotification && (
        <div className="fixed bottom-5 right-5 z-[100] flex items-center gap-3 bg-slate-900 border border-slate-700 text-white py-3.5 px-5 rounded-2xl shadow-2xl animate-in fade-in slide-in-from-bottom-5 duration-300">
          <div className={`p-1.5 rounded-lg ${vaultNotification.type === 'success' ? 'bg-emerald-600' : 'bg-red-600'}`}>
            <Database className="h-4 w-4 text-white" />
          </div>
          <div>
            <p className="text-[10px] text-emerald-400 font-bold uppercase tracking-wider">System Notification</p>
            <p className="text-xs font-bold text-gray-100 mt-0.5">{vaultNotification.message}</p>
          </div>
        </div>
      )}
      ...
      <section className="hidden print:block bg-white text-slate-900 p-0 m-0">
        ...
        <div className="bg-white flex flex-col justify-between" style={{ height: "245mm", maxHeight: "245mm", boxSizing: "border-box", pageBreakAfter: "always", breakAfter: "page", overflow: "hidden", display: "flex", flexDirection: "column" }}>
          ...
          <div className="pt-2 border-t border-gray-200 flex justify-between items-center text-[9px] text-gray-400 font-bold uppercase pb-1">
            <span>CLIENT NUTRITION RECORD - CONFIDENTIAL METRICS</span>
        ...
        <div className="bg-white flex flex-col justify-between pt-2.5" style={{ height: "245mm", maxHeight: "245mm", boxSizing: "border-box", overflow: "hidden", display: "flex", flexDirection: "column" }}>
          <div>
            {/* Printable Report Header */}
            <div className="border-b-4 border-slate-900 pb-2.5 flex items-center justify-between">
              <div>
            ...
            {/* Clean inline banner on PDF for next week targets */}
            {(() => {
              const target = getActiveNextWeekTarget();
              const pGrams = Math.round((target.calories * target.proteinPct / 100) / 4);
              const cGrams = Math.round((target.calories * target.carbsPct / 100) / 4);
              const fGrams = Math.round((target.calories * target.fatPct / 100) / 9);
              return (
                <div className="mt-3.5 border border-slate-900 rounded-xl bg-slate-50 p-2 text-slate-900">
                  <div className="flex justify-between items-center border-b border-gray-200 pb-0.5">
                    <span className="text-[9px] font-black uppercase tracking-widest text-slate-900">FOR NEXT WEEK: PRESCRIBED TARGET METRICS SETUP</span>
                    <span className="text-[8px] font-bold text-slate-500 uppercase tracking-wider">Metabolic Target Plan</span>
                  </div>
                  <div className="grid grid-cols-4 gap-2 text-center mt-1.5 text-[9px] font-bold">
                    <div className="bg-white border border-gray-200 p-1 rounded">
                      <span className="text-gray-400 uppercase text-[7px] block leading-none">ENERGY TARGET</span>
                      <span className="text-slate-950 font-black text-sm block mt-0.5 leading-none">{target.calories} kcal/day</span>
                    </div>
                    <div className="bg-white border border-gray-200 p-1 rounded">
                      <span className="text-blue-600 uppercase text-[7px] block leading-none">PROTEIN TARGET</span>
                      <span className="text-slate-950 font-black text-sm block mt-0.5 leading-none">{target.proteinPct}% <span className="text-[8.5px] text-gray-400 font-semibold">({pGrams}g)</span></span>
                    </div>
                    <div className="bg-white border border-gray-200 p-1 rounded">
                      <span className="text-emerald-700 uppercase text-[7px] block leading-none">CARB VALUE</span>
                      <span className="text-slate-950 font-black text-sm block mt-0.5 leading-none">{target.carbsPct}% <span className="text-[8.5px] text-gray-400 font-semibold">({cGrams}g)</span></span>
                    </div>
                    <div className="bg-white border border-gray-200 p-1 rounded">
                      <span className="text-amber-650 uppercase text-[7px] block leading-none">FAT TARGETS</span>
                      <span className="text-slate-950 font-black text-sm block mt-0.5 leading-none">{target.fatPct}% <span className="text-[8.5px] text-gray-400 font-semibold">({fGrams}g)</span></span>
                    </div>
                  </div>
                  {target.notes && (
                    <div className="mt-2 pt-1.5 border-t border-gray-200 text-[8.5px] text-slate-800 leading-normal text-left font-medium">
                      <span className="font-bold text-slate-900 uppercase text-[7px] tracking-wider block mb-0.5">Coaching Target Notes</span>
                      {target.notes}
                    </div>
                  )}
                </div>
          ...
          {/* Footer for Page 2 */}
          <div className="pt-2 border-t border-gray-200 flex justify-between items-center text-[9px] text-gray-400 font-bold uppercase pb-1">
            <span>CLIENT NUTRITION RECORD - CONFIDENTIAL SUMMARY</span>
